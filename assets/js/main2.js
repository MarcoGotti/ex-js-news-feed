...
const articles = [
  {
    ...
    content:
      "Un breve articolo sulla recente scoperta di una nuova specie di papera di gomma mai vista prima.",
    tags: "geo, tech",
    author: "Diana Rossi",
...
function renderArticles(newsList, domElement) {
  newsList.forEach((news) => {
    ...
    const tagsBoxEl = document.querySelector(
      `[data-index="${news.id}"] .tagsBox`
    );
...
function generateArticle(news) {
  return `            
    <div class="col">
        <div class="box bg-white position-relative" data-index="${news.id}">
            <div>
                ...
                <span class="date">in data ${news.published
                  .split("-")
                  .reverse()
                  .join("/")}</span>
    ...
    news.image === undefined ? "&#169; Missing Image" : ""
  }">
            </div>                        
            <div class="tagsBox">           
            </div>                       
